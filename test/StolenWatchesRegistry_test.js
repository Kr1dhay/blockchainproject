...
describe("StolenWatchesRegistry Contract", function () {
  ...
  beforeEach(async function () {
    // Get signers
    [owner, minter, watchOwner, nonOwner] = await ethers.getSigners();
    ...
    // Add verification (Changed variable name from 'owner' to 'tokenOwner')
  ...
  describe("Flagging watches as stolen", function () {
    it("Should allow token owner to flag watch as stolen", async function () {
    ...
    it("Should not allow non-owner to flag watch as stolen", async function () {
      await expect(
  ...
  describe("Unflagging watches as not stolen", function () {
    ...
    it("Should allow token owner to unflag watch as stolen", async function () {
  ...
  describe("Change of ownership", function () {
    ...
    it("Should allow new owner to unflag a watch after transfer", async function () {
    ...
    it("Should prevent previous owner from unflagging after transfer", async function () {
      // Transfer the NFT to nonOwner
      await luxuryWatchNFT.connect(watchOwner).transferFrom(watchOwner.address, nonOwner.address, tokenId);
      
      // Previous owner should no longer be able to unflag
      await expect(
        stolenWatchesRegistry.connect(watchOwner).unflagAsStolen(tokenId)
      ).to.be.revertedWith("Only the owner of the token can call this function");
    });
